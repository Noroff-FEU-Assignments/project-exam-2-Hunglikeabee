...
export default function Activities() {
    ...
    const getActivities = async () => {
      try {
        const response = await axios.get(
          "https://noroffcors.herokuapp.com/https://www.visitbergen.com/ting-a-gjore/aktiviteter"
        );
  ...
  return (
    <StyledContainer>
      <Subheading>DISCOVER BERGEN</Subheading>
      {error && (
        <DisplayMessage>
          There seems to be an error retrieving activities: {error}
        </DisplayMessage>
      )}
      <StyledActivitiesContainer>
        {bergenActivities ? (
          bergenActivities.map((activity, key) => {
            return (
              <StyledActivityContainer
                target="_blank"
                key={key}
              ...
              >
                <StyledImage
                  style={{ backgroundImage: `url(${activity.fullImage})` }}
                />
